## ---- test-fitDTVARMxID-mu-eta-fixed-false
lapply(
  X = 1,
  FUN = function(i,
                 text) {
    ...
    mu_eta <- fitDTVARMxID:::.FitDTVARMxIDMuEta(
      ...
      center = FALSE,
      mu_eta_fixed = FALSE,
      mu_eta_free = rep(x = TRUE, times = k),
      mu_eta_values = rep(x = 1, times = k),
      mu_eta_lbound = rep(x = -1, times = k),
      mu_eta_ubound = rep(x = +1, times = k),
      name_mu_eta = "mu_eta",
      name_alpha = "alpha",
      name_beta = "beta"
    )
  ...
  text = "test-fitDTVARMxID-mu-eta-fixed-false"
)
