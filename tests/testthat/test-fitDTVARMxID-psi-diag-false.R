## ---- test-fitDTVARMxID-psi-diag-false
lapply(
  X = 1,
  FUN = function(i,
                 text) {
    ...
    testthat::test_that(
      ...
      {
        testthat::skip_on_cran()
        psi_l_values_results <- matrix(
          data = 1,
          nrow = k,
          ncol = k
        )
        psi_l_values_results[
          upper.tri(
            x = psi_l_values_results,
            diag = TRUE
          )
        ] <- 0
        testthat::expect_true(
    ...
    testthat::test_that(
      ...
      {
        testthat::skip_on_cran()
        psi_l_lbound_results <- matrix(
          data = -1,
          nrow = k,
          ncol = k
        )
        psi_l_lbound_results[
          upper.tri(
            x = psi_l_lbound_results,
            diag = TRUE
          )
        ] <- NA
        testthat::expect_true(
    ...
    testthat::test_that(
      ...
      {
        testthat::skip_on_cran()
        psi_l_ubound_results <- matrix(
          data = 1,
          nrow = k,
          ncol = k
        )
        psi_l_ubound_results[
          upper.tri(
            x = psi_l_ubound_results,
            diag = TRUE
          )
        ] <- NA
        testthat::expect_true(
  ...
  text = "test-fitDTVARMxID-psi-diag-false"
)
